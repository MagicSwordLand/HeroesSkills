...
public class EquipAblePassiveStat extends MythicActiveSkill{
    ...
    public EquipAblePassiveStat(String mythicSkill, String display, int maxLevel, double manaConsume, double cooldown, BiFunction<PlayerSkillProfile, SkillData, Icon> icon,
                                BiFunction<PlayerSkillProfile,SkillData, Map<String,Double>>  amountFunction) {
    ...
    public void onUnEquip(@NotNull PlayerSkillProfile skillProfile, SkillData skillData){
        ...
        if(stats != null){
            ...
            stats.forEach(stat->statMap.getInstance(stat).remove("HeroesSkill-"+skillID));
        }
